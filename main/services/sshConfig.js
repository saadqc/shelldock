...
function loadSshConfig(logDebug) {
  const config = { hosts: {}, defaults: {} };
  const configPath = path.join(os.homedir(), '.ssh', 'config');
  if (typeof logDebug === 'function') {
    logDebug('ssh config path', configPath, 'exists', fs.existsSync(configPath));
  }
  parseSshConfigFile(configPath, config);
  return config;
}

function listConfigHosts(logDebug) {
  const config = loadSshConfig(logDebug);
  ...
  if (typeof logDebug === 'function') {
    logDebug('ssh hosts found', entries.length);
  }
  return entries;
}

function resolveHost(alias, logDebug) {
  const config = loadSshConfig(logDebug);
  const values = config.hosts[alias];
  if (!values) {
    return null;
  }
  return { alias, ...config.defaults, ...values };
}
